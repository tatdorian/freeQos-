...
async def test_vues_du_tableau_de_bord(database: Database, now: datetime) -> None:
    ...
    # Les deux echantillons d'un meme abonne doivent tomber dans la MEME tranche
    # de 5 minutes : plus bas, le test affirme que la serie n'en compte qu'une.
    # date_bin aligne ses tranches sur des frontieres absolues (15:20, 15:25...),
    # donc un 'now - 20 s' brut bascule dans la tranche precedente chaque fois
    # que l'heure tombe dans les 20 premieres secondes d'une tranche. C'etait un
    # echec une fois sur quinze environ, sans aucun rapport avec le code teste.
    debut_tranche = now.replace(minute=(now.minute // 5) * 5, second=0, microsecond=0)
    if now - debut_tranche < timedelta(seconds=1):
        # Pile sur une frontiere : il n'y aurait pas la place de poser deux
        # horodatages distincts avant elle.
        now = debut_tranche + timedelta(seconds=1)
    anterieur = now - min(timedelta(seconds=20), now - debut_tranche)

    await writer.write_subscriber_metrics(
        [
            (a, sample(anterieur, 1e6, 10e6)),
            (a, sample(now, 2e6, 20e6)),
            (b, sample(anterieur, 5e6, 50e6)),
